...
#ifndef HDDAQ__HULRESETTIME_UNPACKER_HH
#define HDDAQ__HULRESETTIME_UNPACKER_HH
...
class HulResetTime : public DAQNode, public Uncopyable<HulResetTime>
{
public:
  ...
  static const uint32_t k_HEADER_MAGIC    = 0xffff0200;
  ...
  // RTR -------------------------------------------------------------------
  static const uint32_t k_reset_time_block_mask  = 0xf;
  static const uint32_t k_reset_time_block_shift = 28U;

  static const uint32_t k_reset_time_data_mask  = 0xfffffff;
  static const uint32_t k_reset_time_data_shift = 0U;

private:
  Header*  m_header;
  iterator m_body_first;
  bool     m_has_rm;
  uint32_t m_n_reset_time_block;

public:
  HulResetTime( const unpacker_type& type );
  virtual ~HulResetTime();
