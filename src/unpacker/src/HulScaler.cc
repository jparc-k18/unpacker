...
const defines::unpacker_type HulScaler::k_type = "HulScaler";

//______________________________________________________________________________
HulScaler::HulScaler( const unpacker_type& uType )
  : DAQNode(uType),
    m_header(0),
    m_body_first(),
    m_has_rm(false),
...
//______________________________________________________________________________
HulScaler::~HulScaler( void )
{
}

//______________________________________________________________________________
void
HulScaler::decode( void )
{
  ...
  const std::size_t scaler_channel = k_n_one_block*m_n_scaler_block;
  for( unsigned int ch=0; ch<scaler_channel; ++ch, ++i){
    // unsigned int block = (*i>>k_scaler_block_shift) & k_scaler_block_mask;
    unsigned int val  = (*i>>k_scaler_data_shift)  & k_scaler_data_mask;
    fill( ch, k_scaler, val );
  }
...
//______________________________________________________________________________
void
HulScaler::check_data_format( void )
{
