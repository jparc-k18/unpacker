...
HulMHTdc::decode( void )
{
  ...
  if( m_has_rm ){
    unsigned int event = (*itr_body >> k_rm_event_shift) & k_rm_event_mask;
    unsigned int spill = (*itr_body >> k_rm_spill_shift) & k_rm_spill_mask;
    unsigned int sinc  = (*itr_body >> k_rm_sinc_shift)  & k_rm_sinc_mask;
    unsigned int lock  = (*itr_body >> k_rm_lock_shift)  & k_rm_lock_mask;
  ...
  for(; itr_body != m_data_last; ++itr_body){
    unsigned int data_type = (*itr_body >> k_data_header_shift) & k_data_header_mask;
    unsigned int ch        = (*itr_body >> k_tdc_ch_shift)      & k_tdc_ch_mask;
    unsigned int val       = (*itr_body >> k_tdc_data_shift)    & k_tdc_data_mask;
